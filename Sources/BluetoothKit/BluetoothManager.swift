...
internal class BluetoothManager: NSObject, @unchecked Sendable {
    ...
    public func startEEGDataCollection() {
        ...
        // 데이터 수집 시작 명령 전송
        let startCommand: [UInt8] = [0x01]  // 데이터 수집 시작 명령
        peripheral.writeValue(Data(startCommand), for: characteristic, type: .withResponse)
    ...
    public func stopEEGDataCollection() {
        ...
        // 데이터 수집 중지 명령 전송
        let stopCommand: [UInt8] = [0x00]  // 데이터 수집 중지 명령
        peripheral.writeValue(Data(stopCommand), for: characteristic, type: .withResponse)
